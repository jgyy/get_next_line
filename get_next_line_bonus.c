...
/*   Updated: 2023/09/25 20:07:58 by jegoh            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static t_gnl	gnl[8192];
	char			*line;

	if (fd < 0 || BUFFER_SIZE <= 0)
		return (0);
	gnl[fd].left_str = ft_accumulate_unprocessed_data(fd, &gnl[fd]);
	if (!gnl[fd].left_str)
		return (NULL);
	line = ft_assemble_line_from_buffer(&gnl[fd]);
	gnl[fd].left_str = ft_generate_remaining_string(&gnl[fd]);
	return (line);
}
/*
#include <fcntl.h>
